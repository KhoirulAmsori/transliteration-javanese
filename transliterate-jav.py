...
def ganti (inp, index, character):
	return inp[0:int(index)] + character # + this.subcontent_split(index+character.length)
def ganti2 (inp, index, character):
	return inp[0:index-1] + character # + this.subcontent_split(index+character.length);
def ganti3 (inp, index, character):
	return inp[0:index-2] + character # + this.subcontent_split(index+character.length);
		...
		if ("ꦂ" in regexp_file and regexp_file["ꦂ"] == "r"): #jawa->latin
			if (content_split[i] == "ꦲ"): #ha
				...
				if i > 2 and content_split[i-1] == "ꦲ" and content_split[i-2] == "ꦲ": #-hah-
					...
					#content_split[i] == "ꦶ" or content_split[i] == "ꦸ" or content_split[i] == "ꦺ" or content_split[i] == "ꦼ"
				...
				if i > 0 and content_split[i-1] == "ꦗ":
					...
					if i > 1 and content_split[i-2] == "꧊":
						...
						trans = ganti3(trans, j, "va") #catatan, "va" biasanya ditulis sama dengan "fa" (dengan pa+cecak telu), variannya adalah wa+cecak telu.
				else:
					trans = ganti2(trans, j, regexp_file[content_split[i]])
				...
				trans = ganti(trans, j, regexp_file[content_split[i]]);j+=2;
				...
				if i > 0 and content_split[i-1] == "꧊":
					...
					trans = ganti(trans, j, regexp_file[content_split[i]])
					j+=3
			#/*} elif (content_split[i] == "꧈" or content_split[i] == "꧉") { # habis titik atau koma diberi spasi
			#	trans = ganti(trans, j, regexp_file[content_split[i]]+" ");j+=2;*/
